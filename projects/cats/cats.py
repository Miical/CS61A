...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0:
        ...
        return 100.0 / len(typed_words) * sum([a == b for a, b in\
          zip(typed_words, reference_words)])
...
def faster_autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word 
    if (user_word, diff_function) in mem_dict:
        return mem_dict[user_word, diff_function]

    def get_val(word):
        return diff_dict[(user_word, word)]

    word = min(valid_words, key=get_val)
    mem_dict[user_word, diff_function] = word if get_val(word) <= limit else user_word 
    return mem_dict[user_word, diff_function]
